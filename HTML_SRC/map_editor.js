...
function add_line_arr(xy_arr, color_line, weight_line){
  ...
  highlight_features(myLines, map_editor, color_line, weight_line)
...
var geojson;
function highlight_features(your_features, map_name, color_line, weight_line){
	...
	function StyleSelector(feature, layer) {
		  layer.on({
	...
	geojson = L.geoJson(your_features, {
		  style: style,
		  onEachFeature: StyleSelector
	}).addTo(map_name);
}